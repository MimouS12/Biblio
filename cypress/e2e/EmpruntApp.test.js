...
// houni chna3mlou scénario de réussite ta3 emprunt depuis la page connexion
   it("successfully display the login page and do emprunt", () => {
       //nemchiw lel page login
       cy.visit("/login")
       //l'url lezmha te5ou login
       cy.url().should("include", "/login")
  ...
  .should(($div) => {
    expect($div).to.have.text('Jamais sans ma fille')
      ...
      // yekliki 3al bouton emprunter w lezmou yet2aked eli hiya raj3et retourner ba3d manzel aleha 
       cy.contains(/Emprunter/i).click()
        ...
        expect(str).to.equal(`you just emprunted book [Jamais sans ma fille]`)

       // you just emprunted book ["+libelle+"]
      }) 
    ...
    it.only("successfully display the scénario d'échec : you cannot emprunt more than 2 books ", () => {
      //nemchiw lel page login
      cy.visit("/login")
      //l'url lezmha te5ou login
      cy.url().should("include", "/login")
     ...
     // yekliki 3al bouton emprunter w lezmou yet2aked eli hiya raj3et retourner ba3d manzel aleha 
      ...
      cy.get('@alerted')
      .should('have.been.calledWith', 'you just emprunted book [Jamais sans ma fille]')
      cy.get("[data-testid=Retourner]").should("contain", "Retourner") 


      //cy.findByTestId("Emprunter").click()
      cy.contains(/Emprunter/i).click()

      cy.get('@alerted')
      .should('have.been.calledWith', 'you just emprunted book [Jane Eyre]')
      cy.get("[data-testid=Retourner]").should("contain", "Retourner") 
      ...
      //cy.findByTestId("Emprunter").click()
      cy.get('@alerted')
      .should('have.been.calledWith', 'you cannot emprunt more then 2 books !')
